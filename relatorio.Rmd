...
# 1.2 Analise da variavel código da prova
...
# O símbolo %>% (pipe) significa "e então..."
tabela_resumo_estados <- dados %>%
  
  # PASSO 1: FILTRAR
  # Vamos usar a nota de Ciências Humanas (NU_NOTA_CH)
  # Filtra apenas quem estava PRESENTE e tinha nota
  ...
  # Cria o sumário com as colunas que você pediu
  summarise(
    ...
    N_Alunos = n() # Adicionei a contagem de alunos (N)
  ) %>%
  
  # PASSO 4: ORDENAR (Opcional, mas ajuda)
  # Ordena pela Média, da maior para a menor
  arrange(desc(Media))

# 3. Exibir a tabela formatada
# (Usando kable e kableExtra para ficar bonito)
tabela_resumo_estados %>%
  kable(
    caption = "Tabela Resumo: Estatísticas das Notas de CH por Estado",
    col.names = c("Estado", "Média", "Mediana", "Variância", "Desv. Padrão", "Mínimo", "Máximo", "Nº de Alunos"),
    digits = 2, # Arredonda os números para 2 casas decimais
    format = "html" # Formato para o RMarkdown
  ) %>%
  kable_styling(
...
```{r analise-medidas-estatistica, echo=FALSE}
# 1. Carregar o arquivo de dados
